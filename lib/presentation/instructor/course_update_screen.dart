...
class CourseUpdateScreen extends StatefulWidget {
  ...
  const CourseUpdateScreen({super.key, required this.course});
...
class _CourseUpdateScreenState extends State<CourseUpdateScreen> {
  ...
  void _initializeFields() {
    ...
    _priceController.text = (widget.course['price'] ?? 0.0).toString();

    // Set the current category ID
    final currentCategoryId =
        widget.course['categoryId'] ?? widget.course['category_id'];
    if (currentCategoryId != null) {
      _selectedCategoryId = currentCategoryId is String
          ? int.tryParse(currentCategoryId)
          : currentCategoryId as int?;
    }
  }
  ...
  Future<void> _updateCourse() async {
    ...
    try {
      ...
      await CoursesService.updateCourse(
          widget.course['id'],
          await CoursesService.updateCourse(widget.course['id'], {
